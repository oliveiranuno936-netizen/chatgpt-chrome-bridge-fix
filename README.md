...
**当前版本**：v0.3.0（2026-09-19；更新内容见 [CHANGELOG.md](CHANGELOG.md)）
...
3. `[3/4]` 关闭 ChatGPT → 换入新目录 → 重新启动（应用的 startup reconcile 会立刻校验）
4. `[4/4]` 检查日志是否出现"复用现有市场"事件；未命中则换下一组内部参数重来（共 4 组候选）
...
- **依赖应用内部实现**：缓存标记的组成、复用校验条件、插件清单的过滤规则与日志事件名都取自当前应用版本
  （已验证到 `26.915.4065.0` / 插件 `26.915.31945`）。应用大版本更新后可能失效 ——
  脚本会明确报"未命中复用分支"，而不是静默"假装修好"。v0.3.0 就是为新版本做的适配：
  新构建在标记里多了一个展示开关字段，并且要求运行目录里的技能文件内容与包内逐字节一致。
